...
class Settings:
    ...
    # =========================
    lm_base_env: str = os.getenv("AZURE_API_BASE")
    """
    ...
    lm_key_env: str = os.getenv("AZURE_API_KEY")
    """
    ...
    lm_model_env: str = os.getenv("MODEL")
    """
...
def get_llm(settings: Settings):
    ...
    """
    try:
        base = settings.lm_base_env
        key = settings.lm_key_env
        # model_name = settings.lm_model_env
        ...
        llm = init_chat_model("azure_openai:gpt-4o",api_key=key, api_version=os.getenv("AZURE_API_VERSION"), azure_endpoint=base)
    ...
    for q in questions:
        ...
        if llm:
            try:
                ...
                print(f"{format_docs_for_prompt(hits)}\n")
        else:
